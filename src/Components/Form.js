...
import { Button, FormData } from "./StyleComponents";
...
function DevForm() {
  ...
  return (
    <FormData onSubmit={handleSubmit}>
      ...
      {nameMessageError && <span>{nameMessageError}</span>}
      <div>
        <label htmlFor="phone">Phone</label>
        <input id="phone" type="text" name="phone" value={user.phone} />
        {phoneMessageError && <span>{phoneMessageError}</span>}
      </div>
      <Button type="submit" colorTheme data-clipboard-text="aaaa">
        Send
      </Button>
      <Button>Opa</Button>
    </FormData>
